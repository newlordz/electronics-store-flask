...
def add_to_cart_route(product_id):
    ...
    logger.info(f"🛒 Added product {product.name} to cart for user {session['user_id']}")
    return redirect(url_for('product_detail', product_id=product_id))
...
def remove_from_cart(item_id):
    if item_id in cart_items:
        ...
        logger.info(f"🗑️ Removed item {item_id} from cart for user {session['user_id']}")
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        logger.info(f"📦 Order {order.id} placed successfully for user {session['user_id']}")
...
def buyer_dashboard():
    ...
    logger.debug(f"📊 Buyer dashboard loaded for user {session['user_id']}, {len(user_orders)} orders")
...
def update_order_status(order_id, status):
    ...
    if user and (user.role == 'admin' or (user.role == 'seller' and status == 'delivered')):
        ...
        flash(f'Order status updated from {old_status} to {status}.', 'success')
        logger.info(f"📋 Order {order_id} status updated from {old_status} to {status} by {user.email}")
    else:
        flash('Access denied.', 'danger')
        logger.warning(f"❌ Unauthorized status update attempt by {user.email if user else 'unknown'}")
